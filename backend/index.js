...
const { insertUsers } = require('./services/items');
const { getUsers } = require('./services/items');
const { getData } = require('./services/items');
const { deleteDataByID } = require('./services/items');
const items = require('./services/items');
const { getColeccion } = require('./services/items');
...
//Insertar datos de usuarios en la base de datos
app.post('/addUser', async function(req, res, next) {
  try {
    const result = await insertUsers(req, res);
    if (result > 0) {
      res.json({ message: 'Datos insertados correctamente' });
    } else {
      res.status(500).json({ error: 'Error al insertar datos' });
